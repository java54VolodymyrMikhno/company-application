...
public class CompanyApplItems {
	...
	public static List<Item> getCompanyItems(Company company, HashSet<String> departments) {
		...
		Item[] items = { 
				Item.of("add employee", CompanyApplItems::addEmployeeSubMenu),
	...
	static void addEmployeeSubMenu(InputOutput io) {
        Item[] items = {
        ...
        Menu menu = new Menu("Add Employee Menu", items);
        menu.perform(io);
    }
	 static void addSalesPerson(InputOutput io) {
	        Employee emp = readEmployee(io);
	        Employee result = getSalesPerson(emp, io);
	        company.addEmployee(result);
	        io.writeLine("Sales Person has been added");
	    }
	 static void addWageEmployee(InputOutput io) {
	        Employee emp = readEmployee(io);
	        Employee result = getWageEmployee(emp, io);
	        company.addEmployee(result);
	        io.writeLine("Wage Employee has been added");
	    }

	    static void addManager(InputOutput io) {
	        Employee emp = readEmployee(io);
	        Employee result = getManager(emp, io);
	        company.addEmployee(result);
